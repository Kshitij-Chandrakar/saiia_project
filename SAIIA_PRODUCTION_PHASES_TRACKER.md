...
```text
[~] Implemented locally - Startup Login Screen is implemented for signed-out/token-expired desktop startup using the provided Figma reference, exact mascot asset, and existing safe preload auth login API; validation passed with frontend Node tests and Vite build; broader startup shell, resume/job-target selection UI, backend session routes, Supabase migrations, C4.4 generation integration, cloud sync engine, billing/admin features, and local/cloud data migration are not implemented
```
