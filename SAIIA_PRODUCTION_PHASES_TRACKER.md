...
```text
[x] Completed locally - Signup, including the Google signup action, requires Terms & Conditions and Privacy Policy acceptance, the marketing checkbox is unchecked by default with tri-state consent (untouched means no preference update), and the authenticated profile-bootstrap path persists consent fields using the verified JWT user identity. The local-only migration `20260904170000_add_signup_consent_preferences.sql` has not been applied remotely. Legacy no-consent bootstrap remains compatible before the migration, but the frontend consent flow is disabled by default and must only be enabled with `VITE_CONSENT_FEATURE_ENABLED=true` after applying `20260904170000_add_signup_consent_preferences.sql` followed by `20260905103000_add_marketing_unsubscribe_tokens.sql`; absent columns must fail safely rather than discard consent or claim it was persisted. Signup `/terms` and `/privacy` links are safe placeholders pending real legal pages. No marketing/promotional sends, unsubscribe flow, Resend calls, SMTP calls, or real emails were added; C10.6B unsubscribe and marketing delivery are not started.
```
...
```text
[ ] Required before public production release - Desktop Website-Login Handoff Production Hardening must replace the C6.2A process-local memory handoff store with shared atomic TTL-backed storage, using either Supabase/Postgres TTL-backed table plus atomic RPC/transaction or Redis atomic TTL operations. The production store must preserve 2-5 minute TTL, one-time exchange, SHA-256 handoff-code hash lookup keys, token-safe URLs/logging/renderer boundaries, atomic create limits/rate limits, atomic exchange/consume behavior, cleanup for expired records, and multi-worker/multi-instance deployment safety.
```
