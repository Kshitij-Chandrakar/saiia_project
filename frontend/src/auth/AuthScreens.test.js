...
test('protected dashboard shows safe user identity and avoids token state', () => {
  ...
  assert.match(source, /askInterviewSessionAI,/)
  assert.match(source, /fetchInterviewAskAIMessages,/)
...
test('dashboard loads session history and transcript controls with separate loading, error, and empty states', () => {
  ...
  assert.match(dashboardPageSource, /const \[openAskAISessionId, setOpenAskAISessionId\] = useState\(''\)/)
  assert.match(dashboardPageSource, /const \[askAIMessages, setAskAIMessages\] = useState\(\[\]\)/)
  assert.match(dashboardPageSource, /const \[askAIMessagesNextPage, setAskAIMessagesNextPage\] = useState\(null\)/)
  assert.match(dashboardPageSource, /const \[askAIDrafts, setAskAIDrafts\] = useState\(\{\}\)/)
  assert.match(dashboardPageSource, /const \[askAILoading, setAskAILoading\] = useState\(false\)/)
  assert.match(dashboardPageSource, /const \[askAIError, setAskAIError\] = useState\(''\)/)
  assert.match(dashboardPageSource, /const askAIMessagesControllerRef = useRef\(null\)/)
  assert.match(dashboardPageSource, /const askAISubmitControllerRef = useRef\(null\)/)
  assert.match(dashboardPageSource, /function resetAskAIState\(\) \{[\s\S]*askAIMessagesControllerRef\.current\?\.abort\(\)[\s\S]*askAISubmitControllerRef\.current\?\.abort\(\)[\s\S]*setAskAIMessagesNextPage\(null\)[\s\S]*setAskAILoading\(false\)/)
  ...
  assert.match(source, /const ASK_AI_CONTEXT_MISSING_ERROR = 'This session does not have transcript or AI notes context yet\.'/)
  assert.match(source, /const ASK_AI_CONTEXT_MISSING_MESSAGE = 'This session does not have transcript or AI notes context yet\. Record interview questions first, then come back to Ask AI\.'/)
  assert.match(source, /function isAskAIContextMissingError\(message\)/)
  assert.match(source, /function cleanNotesMarkdownLine\(line\)/)
  assert.match(source, /function getNotesMarkdownFallbackItems\(notes\)/)
  assert.match(source, /function getStructuredNotesSections\(notes\)/)
  assert.match(source, /function formatAskAIMessageMetaLine\(message\)/)
  ...
  assert.match(dashboardPageSource, /handleAskAIToggle\(session\.id\)/)
  assert.match(dashboardPageSource, /handleAskAILoadMore\(session\.id\)/)
  ...
  assert.match(dashboardPageSource, /getStructuredNotesSections\(sessionNotes\)\.map\(\(section\) =>/)
  assert.match(source, /'summary', 'Summary'/)
  assert.match(source, /'technical_topics', 'Topics Covered'/)
  assert.match(source, /'key_questions', 'Key Questions Asked'/)
  assert.match(source, /'strengths', 'Strong Points'/)
  assert.match(source, /'improvement_areas', 'Areas to Improve'/)
  assert.match(source, /'suggested_followups', 'Suggested Follow-up Practice'/)
  assert.match(source, /title: 'Overall Feedback'/)
  assert.match(dashboardPageSource, /className="auth-session-history__notes-section"/)
  assert.match(dashboardPageSource, /<h4>\{section\.title\}<\/h4>/)
  assert.match(dashboardPageSource, /section\.items\.map\(\(item, index\) => \(/)
  assert.match(dashboardPageSource, /<li key=\{`\$\{section\.key\}-\$\{index\}`\}>\{item\}<\/li>/)
  assert.doesNotMatch(dashboardPageSource, /<li key=\{item\}>\{item\}<\/li>/)
  assert.doesNotMatch(dashboardPageSource, /<pre className="auth-session-history__notes-markdown">/)
  assert.doesNotMatch(dashboardPageSource, /# Interview Notes/)
  assert.doesNotMatch(dashboardPageSource, /## Summary/)
  ...
  assert.match(dashboardPageSource, /async function handleAskAILoadMore\(sessionId\) \{[\s\S]*page: askAIMessagesNextPage/)
  assert.match(dashboardPageSource, /setAskAIMessages\(\(current\) => \{[\s\S]*const seen = new Set\(current\.map\(\(message\) => message\.id\)\)/)
  assert.match(dashboardPageSource, /async function handleAskAISubmit\(event, sessionId\) \{[\s\S]*const controller = new AbortController\(\)[\s\S]*askAISubmitControllerRef\.current = controller[\s\S]*askInterviewSessionAI\(data\.session\.access_token, normalizedSessionId, question/)
  assert.match(dashboardPageSource, /askAISubmitControllerRef\.current !== controller \|\| openAskAISessionId !== normalizedSessionId/)
  assert.match(dashboardPageSource, /setAskAIMessages\(\(current\) => \[[\s\S]*result\.user_message,[\s\S]*result\.assistant_message/)
  assert.match(dashboardPageSource, /const question = String\(askAIDrafts\[normalizedSessionId\] \|\| ''\)\.trim\(\)/)
  assert.match(dashboardPageSource, /setAskAIDrafts\(\(current\) => \(\{[\s\S]*\[normalizedSessionId\]: '',[\s\S]*\}\)\)/)
  assert.match(dashboardPageSource, /const \[askAIRequestIds, setAskAIRequestIds\] = useState\(\{\}\)/)
  assert.match(dashboardPageSource, /requestState\?\.question === question[\s\S]*requestState\.requestId/)
  assert.match(dashboardPageSource, /delete next\[normalizedSessionId\]/)
  assert.match(dashboardPageSource, /delete next\[session\.id\]/)
  assert.match(dashboardPageSource, /if \(isAskAIContextMissingError\(message\)\) \{[\s\S]*setAskAIError\(ASK_AI_CONTEXT_MISSING_MESSAGE\)[\s\S]*setAskAIDrafts\(\(current\) => \(\{[\s\S]*\[normalizedSessionId\]: '',/)
  assert.match(dashboardPageSource, /value=\{askAIDrafts\[session\.id\] \|\| ''\}/)
  assert.match(dashboardPageSource, /\[session\.id\]: event\.target\.value/)
  assert.match(dashboardPageSource, /spellCheck=\{false\}/)
  assert.match(dashboardPageSource, /autoCorrect="off"/)
  assert.match(dashboardPageSource, /autoComplete="off"/)
  assert.match(dashboardPageSource, /placeholder="Ask what to improve, rewrite an answer, or practice follow-ups\.\.\."/)
  assert.match(dashboardPageSource, /Loading Ask AI\.\.\./)
  assert.match(dashboardPageSource, /No Ask AI messages yet\./)
  assert.match(dashboardPageSource, /Could not load Ask AI messages\. Please try again\./)
  assert.match(dashboardPageSource, /Retry Ask AI/)
  assert.match(dashboardPageSource, /handleAskAIToggle\(session\.id, \{ reload: true \}\)/)
  assert.match(dashboardPageSource, /Load more messages/)
  assert.match(dashboardPageSource, /Loading more messages\.\.\./)
  assert.match(dashboardPageSource, /!isAskAIContextMissingError\(askAIError\) \? \([\s\S]*Retry Ask AI/)
  assert.match(dashboardPageSource, /!isAskAIContextMissingError\(askAIError\) \? \([\s\S]*<form className="auth-session-history__ask-ai-form"/)
  assert.match(dashboardPageSource, /Send Ask AI/)
  assert.match(dashboardPageSource, /formatAskAIMessageMetaLine\(message\)/)
  assert.match(dashboardPageSource, /openAskAISessionId === session\.id/)
  assert.match(dashboardPageSource, /askAIMessages\.map\(\(message\) =>/)
  assert.doesNotMatch(dashboardPageSource, /dangerouslySetInnerHTML/)
  ...
  assert.match(cssSource, /\.auth-session-history__transcript-entry \.auth-session-history__line,[\s\S]*\.auth-session-history__ask-ai-message \.auth-session-history__line \{\s+white-space: pre-wrap;/)
  assert.match(cssSource, /\.auth-session-history__ask-ai-form textarea \{[\s\S]*padding: 10px 12px;[\s\S]*color: #1f2937;[\s\S]*line-height: 1\.5;[\s\S]*resize: vertical;/)
  assert.match(cssSource, /\.auth-session-history__notes-sections \{[\s\S]*grid-template-columns: repeat\(auto-fit, minmax\(220px, 1fr\)\);/)
  assert.match(cssSource, /\.auth-session-history__notes-section h4 \{[\s\S]*font-size: 14px;[\s\S]*line-height: 1\.35;/)
  assert.match(cssSource, /\.auth-session-history__notes-section ul \{[\s\S]*padding-left: 18px;[\s\S]*line-height: 1\.55;/)
