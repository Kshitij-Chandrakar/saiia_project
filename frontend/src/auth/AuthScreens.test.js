...
test('dashboard loads session history and transcript controls with separate loading, error, and empty states', () => {
  ...
  assert.match(dashboardPageSource, /<li key=\{item\}>\{item\}<\/li>/)
  ...
  assert.match(dashboardPageSource, /if \(openAskAISessionId === normalizedSessionId && !reload\) \{[\s\S]*askAIMessagesControllerRef\.current\?\.abort\(\)[\s\S]*askAISubmitControllerRef\.current\?\.abort\(\)[\s\S]*setOpenAskAISessionId\(''\)/)
