...
test('dashboard loads session history and transcript controls with separate loading, error, and empty states', () => {
  ...
  assert.match(dashboardPageSource, /async function handleAskAIToggle\(sessionId\) \{[\s\S]*setOpenAskAISessionId\(normalizedSessionId\)[\s\S]*fetchInterviewAskAIMessages\(data\.session\.access_token, normalizedSessionId/)
  assert.match(dashboardPageSource, /if \(openAskAISessionId === normalizedSessionId\) \{[\s\S]*askAIMessagesControllerRef\.current\?\.abort\(\)[\s\S]*setOpenAskAISessionId\(''\)/)
  ...
  assert.match(dashboardPageSource, /async function handleAskAISubmit\(event, sessionId\) \{[\s\S]*event\.preventDefault\(\)[\s\S]*askInterviewSessionAI\(data\.session\.access_token, normalizedSessionId, question/)
