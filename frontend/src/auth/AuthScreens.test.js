...
test('cloud resume page supports delete and rebuild lifecycle controls safely', () => {
  ...
  assert.match(resumePageSource, /async function handleDeleteResume\(\)/)
  assert.match(resumePageSource, /window\.confirm\('Delete this cloud resume\?'\)/)
  assert.match(resumePageSource, /deleteCloudResume\(token, targetResume\.id, \{ backendUrl \}\)/)
  assert.match(resumePageSource, /setCurrentResume\(null\)[\s\S]*setReviewCandidate\(null\)[\s\S]*setResumeRecord\(null\)/)
  assert.match(resumePageSource, /setExtractionAttempt\(null\)[\s\S]*clearSelectedResumeFile\(\)/)
