...
test('startup session setup screen collects local setup and routes back or into runtime', () => {
  ...
  assert.match(startupSetupSource, /const selectedResumeBlocked = Boolean\(selectedResume && selectedResume\.can_generate !== true\)/)
  ...
  assert.match(appSource, /job_context_included: generatePayload\.job_context_included \?\? Boolean\(jobDescription\)/)
  ...
  assert.match(appSource, /generationRequestIncludesSelectedResumeId: Boolean\(generateRequestBody\.selected_resume_id\)/)
  assert.match(appSource, /jobContextIncluded: Boolean\(jobDescription\)/)
