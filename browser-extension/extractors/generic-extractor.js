...
import { emptyAnswer, QUESTION_TYPES } from '../schemas/screen-intelligence-schema.js'
...
const SECTION_LABEL_PATTERN = '(?:problem|statement|question|description|instructions?|task|function description|input format|parameters|arguments|function parameters|standard input|output format|print|prints|return value|returns|standard output|expected return|constraints|limits|example(?:\\s+\\d+)?|examples|sample(?:\\s+\\d+)?|samples|sample input(?:\\s+\\d+)?|sample output(?:\\s+\\d+)?|example input(?:\\s+\\d+)?|example output(?:\\s+\\d+)?|expected output(?:\\s+\\d+)?|example result|explanation(?:\\s+\\d+)?|sample explanation(?:\\s+\\d+)?|example explanation(?:\\s+\\d+)?|test case(?:\\s+\\d+)?|notes?|custom input|test against custom input|test result|run output|your output|console output|console|submit code|run code)'
const EDITOR_RUNTIME_MARKER = '(?:monaco-editor|view-lines|ace_editor|ace_text-layer|codemirror|cm-editor|cm-content|generic[-_ ]?editor|code[-_ ]?editor|editor[-_ ]?(?:root|container|pane|area)|runtime|custom[-_ ]?input|testcase|test[-_ ]?result|run[-_ ]?output|console|editorial|solution|discussion|submissions?|sidebar)'
const EDITOR_RUNTIME_SELECTOR = 'script,style,noscript,template,nav,aside,footer,header,[hidden],[aria-hidden="true"],textarea,[contenteditable="true"],[role="textbox"],[aria-multiline="true"],[class*="editor"],[class*="code-editor"],[class*="monaco"],[class*="ace_"],[class*="CodeMirror"],[class*="cm-editor"],[class*="console"],[class*="testcase"],[class*="test-result"],[class*="run-output"],[class*="solution"],[class*="editorial"],[class*="discussion"]'
const NOISE_BLOCK_RE = /<(nav|aside|footer|header)\b[\s\S]*?<\/\1>|<[^>]+(?:aria-hidden=["']?true["']?|hidden|role=["']?(?:navigation|banner|contentinfo|toolbar|search|menu)["']?)[^>]*>[\s\S]*?<\/[^>]+>/gi
const UNSAFE_BLOCK_RE = /<script\b[\s\S]*?<\/script>|<style\b[\s\S]*?<\/style>|<noscript\b[\s\S]*?<\/noscript>|<template\b[\s\S]*?<\/template>/gi
...
export function sanitizeText(value, limit = LIMITS.maxTextLength) {
  return String(value ?? '')
    .replace(/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/g, ' ')
    .replace(/[ \t\f\v]+/g, ' ')
    .replace(/\s*\n\s*/g, '\n')
...
export function sanitizeCode(value, limit = LIMITS.maxCodeLength) {
  return String(value ?? '')
    .replace(/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/g, ' ')
    .replace(/\r\n?/g, '\n')
    .replace(/\n{4,}/g, '\n\n\n')
...
export function extractFromHtml(html, metadata = {}) {
  resetExampleDiagnostics()
  const safeHtml = stripNoise(stripUnsafeBlocks(String(html ?? '')))
  const candidates = discoverHtmlCandidates(safeHtml)
  const selection = selectCandidate(candidates)
  const regionHtml = selection.selected?.html || safeHtml
  const rawTitle = titleFromRegion(regionHtml) || cleanDocumentTitle(matchFirst(safeHtml, /<title[^>]*>([\s\S]*?)<\/title>/i))
  const sections = extractSectionsFromHtml(regionHtml)
  const editorEvidence = collectEditorEvidenceFromHtmlWithFallback(regionHtml, safeHtml)
  const code = editorEvidence.code
  const constraintInfo = extractConstraintsFromHtml(regionHtml, sections.constraints, code)
  sections.constraints = constraintInfo.text
  const options = extractOptionsFromHtml(regionHtml)
  const detectedOptionCount = countDetectedOptionCandidatesFromHtml(regionHtml)
  const visual = extractVisualContextFromHtml(regionHtml)
  return buildQuestion({
    title: rawTitle,
    bodyText: stripTags(regionHtml),
    sections,
    code,
    editorEvidence,
    constraintInfo,
...
export function extractFromDocument(doc = document) {
  resetExampleDiagnostics()
  const candidates = discoverDocumentCandidates(doc)
  const selection = selectCandidate(candidates)
  const root = selection.selected?.node || doc?.body
  const sections = extractSectionsFromDocument(root)
  const editorEvidence = collectEditorEvidenceFromDocumentWithFallback(root, doc)
  const code = editorEvidence.code
  const constraintInfo = extractConstraintsFromDocument(root, sections.constraints, code)
  sections.constraints = constraintInfo.text
  const options = extractOptionsFromDocument(root)
  const detectedOptionCount = countDetectedOptionCandidatesFromDocument(root)
  const visual = extractVisualContextFromDocument(root)
  const title = titleFromNode(root) || cleanDocumentTitle(doc?.title || '')
  return buildQuestion({
    title,
    bodyText: visibleText(root),
    sections,
    code,
    editorEvidence,
    constraintInfo,
...
function buildQuestion({ title, bodyText, sections, code, editorEvidence = emptyEditorEvidence(), constraintInfo = emptyConstraintInfo(), options, detectedOptionCount = 0, visual, selection }) {
  const statement = pickStatement(sanitizeText((sections.statement || sections.function_description || bodyText).replace(editorEvidence.placeholderText || '', ''), LIMITS.maxTextLength), title)
  const signature = findSignature(code)
  const className = findClassName(code)
  const questionType = normalizeQuestionType(detectQuestionType({ statement, sections, code, options, visual }))
  const finalOptions = questionType === 'mcq' ? options : []
  const completeness = evaluateCompleteness({ questionType, statement, sections, code, options: finalOptions, signature, visual })
  const exampleDiagnostics = collectExampleDiagnostics(sections.examples)
  ...
  const confidence = calculateConfidence({ questionType, statement, title, sections, code, options: finalOptions, visual, selection, complete: completeness.complete, warnings })

  return {
    question: {
      question_type: questionType,
      cleaned_question: buildCleanedQuestion({ title, statement, sections }),
      title: title || null,
      statement: statement || null,
      function_description: sections.function_description || null,
      input_format: sections.input_format || null,
      output_format: sections.output_format || null,
      ...
      code_context: {
        selected_language: detectLanguageCandidate(code),
    ...
    extraction: {
      ...
      selected_candidate_score: selection.selected?.score ?? 0,
      ...
      status: selection.ambiguous ? 'selection_required' : undefined,
    },
  }
}

function countDetectedOptionCandidatesFromHtml(html) {
  return [...String(html || '').matchAll(/<(label|li|div|span)[^>]*(?:role=["']?option["']?|[^>]*)>([\s\S]*?)<\/\1>/gi)]
    .filter((match) => {
      const inner = match[2] || ''
      const text = stripTags(inner)
      return text && !isActionOrControlText(text) && !/<button\b|<select\b|<option\b/i.test(inner)
    })
    .length
}

function countDetectedOptionCandidatesFromDocument(root) {
...
function buildCleanedQuestion({ title, statement, sections }) {
  return sanitizeText([
    title,
    statement,
    sections.function_description,
    sections.input_format,
    sections.output_format,
    sections.constraints,
    ...(sections.examples || []).flatMap((example) => [example.input && `Input: ${example.input}`, example.output && `Output: ${example.output}`]),
  ].filter(Boolean).join('\n'), LIMITS.maxTextLength) || null
}

function stripUnsafeBlocks(html) {
  return html.replace(UNSAFE_BLOCK_RE, ' ')
}

function stripNoise(html) {
  return html
    .replace(NOISE_BLOCK_RE, ' ')
    .replace(/<[^>]*(?:cookie|advert|notification|profile|account|toolbar|pagination|breadcrumb)[^>]*>[\s\S]*?<\/[^>]+>/gi, ' ')
    .replace(/<button\b[\s\S]*?<\/button>/gi, ' ')
}

function discoverHtmlCandidates(html) {
  const blocks = []
  const regionPattern = /<(main|article|section|form|div)[^>]*(?:role=["']?main["']?|data-question|question|problem|challenge|task|content|card|panel)?[^>]*>([\s\S]*?)<\/\1>/gi
  for (const match of html.matchAll(regionPattern)) {
    const candidateHtml = match[0]
    const text = stripTags(candidateHtml)
    const score = scoreCandidateText(text, candidateHtml)
    if (text.length >= 24 || score > 4) {
      blocks.push({ html: candidateHtml, text, score })
    }
  }
  const bodyText = stripTags(html)
  blocks.push({ html, text: bodyText, score: scoreCandidateText(bodyText, html) - 2 })
  return blocks
}

function discoverDocumentCandidates(doc) {
  const nodes = Array.from(doc?.querySelectorAll?.('main,article,section,form,[role="main"],[data-question],div') || [])
    .filter(isVisible)
    .slice(0, 180)
  const candidates = nodes
    .map((node) => {
      const text = visibleText(node)
      return { node, text, score: scoreCandidateText(text, node.outerHTML || '') + viewportBonus(node) }
    })
    .filter((candidate) => candidate.text.length >= 24 || candidate.score > 4)
  if (doc?.body) {
    const text = visibleText(doc.body)
    candidates.push({ node: doc.body, text, score: scoreCandidateText(text, doc.body.outerHTML || '') - 2 })
  }
  return candidates
}
...
function selectCandidate(candidates) {
  const initial = [...candidates]
    ...
    .slice(0, LIMITS.maxCandidateRegions || 12)
  const splitPair = findComplementaryCodingPair(initial)
  const sorted = collapseCandidateOverlaps(initial)
  ...
  const closeScores = Boolean(selected && second && selected.score >= 4 && Math.abs(selected.score - second.score) <= 1.2)
  ...
  return {
    ...
    diagnostics: {
      ...
      collapsed_candidate_count: sorted.length,
      top_score: finalSelected?.score ?? 0,
      second_score: sorted.length > 1 ? sorted[1]?.score ?? null : null,
...
function sharedWorkspaceCandidate(candidates, a, b) {
  return candidates
    ...
    .filter((candidate) => candidate.html && a.html && b.html && candidate.html.includes(a.html) && candidate.html.includes(b.html))
    .filter((candidate) => !isBroadHtmlRoot(candidate.html))
    .sort((left, right) => left.text.length - right.text.length || right.score - left.score)[0] || null
}

function isBroadHtmlRoot(html) {
  return /^<!doctype|<html\b|<body\b/i.test(String(html || '').trim())
}
...
function candidateSignals(candidate) {
  ...
  const html = String(candidate?.html || candidate?.node?.outerHTML || '')
  const code = candidate?.node ? extractCodeFromDocument(candidate.node) : extractCodeFromHtml(html)
...
function scoreCandidateText(text, html = '') {
  const safe = sanitizeText(text)
  let score = 0
  if (/\b(problem|question|task|description|instructions?)\b/i.test(safe)) score += 4
  if (/\b(input format|output format|constraints?|examples?|sample input|sample output)\b/i.test(safe)) score += 3
  if (/\b(function|class|return|print|stdin|stdout|debug|fix|output)\b/i.test(safe)) score += 2
  if (/<(pre|code|textarea)\b|contenteditable=["']?true/i.test(html)) score += 4
  if (/<input\b[^>]*type=["']?(radio|checkbox)|role=["']?option/i.test(html)) score += 4
  if (/<(img|figure|svg|canvas)\b/i.test(html)) score += 2
  if (safe.length > 80) score += 2
  if (safe.length > 400) score += 1
  if (/\b(nav|menu|profile|account|advertisement|copyright|subscribe|share|settings)\b/i.test(safe)) score -= 4
  return score
}

function extractSectionsFromHtml(html) {
  const sections = { statement: '', examples: [] }
  const found = []
  const labelTag = '(?:h[1-6]|strong|b|dt|p|div|span)'
  const cleaned = stripEditorAndRuntimeHtml(html)
  const boundary = `(?:<${labelTag}[^>]*>\\s*${SECTION_LABEL_PATTERN}\\s*:?\\s*<\\/${labelTag}>|<[^>]+(?:class|id|role|aria-label|data-testid|data-cy)=["'][^"']*${EDITOR_RUNTIME_MARKER}[^"']*["'][^>]*>|$)`
  const sectionPattern = new RegExp(`<${labelTag}[^>]*>\\s*(${SECTION_LABEL_PATTERN})\\s*:?\\s*<\\/${labelTag}>([\\s\\S]*?)(?=${boundary})`, 'gi')
  for (const match of cleaned.matchAll(sectionPattern)) {
    const label = sanitizeText(match[1])
    const text = stripTags(match[2]).slice(0, LIMITS.maxSectionTextLength)
    found.push(assignSection(sections, label, text))
  }
  if (!sections.statement) {
    sections.statement = stripTags(cleaned.replace(/<(pre|code|textarea)\b[\s\S]*?<\/\1>/gi, ' '))
  }
  if (!sections.examples.length) {
    sections.examples = extractExamplesFromHtml(html)
  }
  return sections
}

function extractSectionsFromDocument(root) {
  const sections = { statement: '', examples: [] }
  ...
  for (const labelNode of labels) {
    ...
    if (key) {
      assignSection(sections, label, text)
    }
  }
  if (!sections.statement) {
    sections.statement = sanitizeText(visibleText(root).replace(extractCodeFromDocument(root), ''), LIMITS.maxTextLength)
  }
  if (!sections.examples.length) {
    sections.examples = extractExamplesFromDocument(root)
  }
  return sections
}

function assignSection(sections, label, text, labelIsKey = false) {
  const key = labelIsKey ? label : labelKey(label)
  if (!key || !text) return null
  if (key === 'examples') {
    sections.examples = limitExamples(mergeExamples(sections.examples, parseExamples(text, parseExampleHeading(label))))
    return key
  }
  if (key === 'runtime_panel') return key
  sections[key] = sections[key] || sanitizeText(text, LIMITS.maxSectionTextLength)
  return key
}

function labelKey(label) {
  const safe = sectionLabelPrefix(label)
  const entry = SECTION_DEFS.find(([, pattern]) => pattern.test(safe))
  return entry?.[0] || null
}
...
function isEditorRuntimeSolutionNode(node) {
  ...
  return new RegExp(EDITOR_RUNTIME_MARKER, 'i').test(marker)
}

function cleanSectionNodeText(node) {
  if (!node?.cloneNode) return ''
  const clone = node.cloneNode(true)
  const excluded = clone.querySelectorAll?.(EDITOR_RUNTIME_SELECTOR) || []
  exampleDiagnosticsState.editorSectionExcludedCount += excluded.length
  excluded.forEach((entry) => entry.remove())
  return sanitizeTextPreserveTabs(clone.textContent || '', LIMITS.maxSectionTextLength)
}
...
function collectLabelledText(labelNode, key = labelKey(nodeLabel(labelNode))) {
  ...
  while (node && chunks.length < 12) {
    ...
    if (isVisible(node)) chunks.push(node.matches?.('table') ? visibleTableText(node) : cleanSectionNodeText(node))
...
function extractCodeFromHtml(html) {
  return collectEditorEvidenceFromHtml(html).code
}

function extractCodeFromDocument(root) {
  return collectEditorEvidenceFromDocument(root).code
}
...
function collectEditorEvidenceFromHtml(html) {
  const source = String(html || '')
  const candidates = []
  const editorRootCount = countHtmlEditorRoots(source)
  const folded = hasFoldedEditorMarkup(source)

  addHtmlLineCandidate(candidates, source, 'monaco', 'monaco_view_lines', /<[^>]*class=["'][^"']*\bview-line\b[^"']*["'][^>]*>([\s\S]*?)<\/[^>]+>/gi, folded)
  addHtmlLineCandidate(candidates, source, 'ace', 'ace_text_layer', /<[^>]*class=["'][^"']*\bace_line\b[^"']*["'][^>]*>([\s\S]*?)<\/[^>]+>/gi, folded)
  addHtmlLineCandidate(candidates, source, 'codemirror6', 'codemirror6_lines', /<[^>]*class=["'][^"']*\bcm-line\b[^"']*["'][^>]*>([\s\S]*?)<\/[^>]+>/gi, folded)
  addHtmlLineCandidate(candidates, source, 'codemirror5', 'codemirror5_lines', /<[^>]*class=["'][^"']*\bCodeMirror-line\b[^"']*["'][^>]*>([\s\S]*?)<\/[^>]+>/gi, folded)
  if (editorRootCount > 0) {
    addHtmlLineCandidate(candidates, source, 'generic_editor', 'generic_editor_lines', /<[^>]*(?:class=["'][^"']*\b(?:editor-line|code-line)\b[^"']*["']|data-line-number=["'][^"']*["'])[^>]*>([\s\S]*?)<\/[^>]+>/gi, folded)
    for (const match of source.matchAll(/<([a-z][\w:-]*)[^>]*(?:class|id)=["'][^"']*\b(?:generic-editor|code-editor|editor-pane|editor-container|editor-area)\b[^"']*["'][^>]*>([\s\S]*?)<\/\1>/gi)) {
      addCodeCandidate(candidates, stripTags(match[2] || ''), 'generic_editor', 'generic_editor_text', true, 6)
    }
  }

  for (const match of source.matchAll(/<textarea[^>]*>([\s\S]*?)<\/textarea>/gi)) {
    addCodeCandidate(candidates, stripCodeTags(match[1] || ''), 'native_textarea', 'textarea_value', true, 8)
  }
  for (const match of source.matchAll(/<[^>]*contenteditable=["']?true["']?[^>]*>([\s\S]*?)<\/[^>]+>/gi)) {
    addCodeCandidate(candidates, stripCodeTags(match[1] || ''), 'contenteditable', 'contenteditable_text', true, 7)
  }
  for (const match of source.matchAll(/<(pre|code)[^>]*>([\s\S]*?)<\/\1>/gi)) {
    const context = source.slice(Math.max(0, match.index - 300), Math.min(source.length, (match.index || 0) + (match[0] || '').length + 300))
    if (/\b(example|sample|output|console|testcase|explanation|editorial|solution)\b/i.test(stripTags(context)) && !/\b(starter|template|your code|complete|implement|function|class|main|following code|printed by|what output)\b/i.test(stripTags(context))) continue
    addCodeCandidate(candidates, stripCodeTags(match[2] || ''), 'preformatted_code', 'pre_code_block', false, 4)
  }

  return finalizeEditorEvidence({ candidates, editorPresent: editorRootCount > 0, editorRootCount })
}

function collectEditorEvidenceFromHtmlWithFallback(regionHtml, fullHtml) {
  const selected = collectEditorEvidenceFromHtml(regionHtml)
  ...
  const fullRootCount = countHtmlEditorRoots(fullHtml)
  if (fullRootCount === 1) {
    const fallback = collectEditorEvidenceFromHtml(fullHtml)
    if (fallback.codeAvailable || fallback.editorTextAvailable || fallback.editorPresent) {
      return { ...fallback, editorScope: 'document_single_editor_fallback', editorRootCount: fullRootCount }
    }
  }
  if (fullRootCount > 1) return { ...selected, editorScope: 'ambiguous_multiple_editors' }
  return { ...selected, editorScope: selected.editorPresent ? 'selected_root' : 'unavailable' }
}

function collectEditorEvidenceFromDocument(root) {
  const candidates = []
  const editorRoots = collectEditorRoots(root)
  for (const editorRoot of editorRoots) {
    const editorType = detectEditorTypeFromNode(editorRoot)
    ...
    addNodeLineCandidate(candidates, editorRoot, '.view-line', editorType || 'monaco', 'monaco_view_lines')
    addNodeLineCandidate(candidates, editorRoot, '.ace_line', editorType || 'ace', 'ace_text_layer')
    addNodeLineCandidate(candidates, editorRoot, '.cm-line', editorType || 'codemirror6', 'codemirror6_lines')
    addNodeLineCandidate(candidates, editorRoot, '.CodeMirror-line', editorType || 'codemirror5', 'codemirror5_lines')
    addNodeLineCandidate(candidates, editorRoot, '.editor-line,.code-line,[data-line-number]', editorType || 'generic_editor', 'generic_editor_lines')
  ...
  collectSameOriginFrameDocuments(root).forEach((doc) => {
    const nested = collectEditorEvidenceFromDocument(doc.body)
...
function collectEditorEvidenceFromDocumentWithFallback(root, doc) {
  const selected = collectEditorEvidenceFromDocument(root)
  if (selected.codeAvailable) return { ...selected, editorScope: 'selected_root' }
  if (selected.editorBoilerplateOnly) return { ...selected, editorScope: 'selected_root' }
  const documentRoots = collectEditorRoots(doc?.body)
  ...
  const fallbackEvidence = documentRoots.map((node) => collectEditorEvidenceFromDocument(node))
...
function addHtmlLineCandidate(candidates, html, editorType, method, pattern, partial = false) {
  const lines = [...html.matchAll(pattern)].map((match) => decodeCodeFragment(match[1] || ''))
  if (lines.length) addCodeCandidate(candidates, lines.join('\n'), editorType, method, true, 10, partial)
}

function addNodeLineCandidate(candidates, root, selector, editorType, method) {
...
function countHtmlEditorRoots(html) {
  const concreteRoots = (html.match(/\b(monaco-editor|ace_editor|CodeMirror|cm-editor)\b/gi) || []).length
  const genericRoots = concreteRoots ? 0 : (html.match(/\b(generic-editor|code-editor|editor-pane|editor-container|editor-area)\b/gi) || []).length
  const nativeRoots = concreteRoots ? 0 : countNativeHtmlEditorRoots(html)
  return concreteRoots + genericRoots + nativeRoots
}

function countNativeHtmlEditorRoots(html) {
  let count = 0
  for (const match of String(html || '').matchAll(/<textarea([^>]*)>([\s\S]*?)<\/textarea>/gi)) {
    const attrs = match[1] || ''
    const value = stripCodeTags(match[2] || '')
    if (/\b(code|editor)\b/i.test(attrs) || looksLikeCode(value)) count += 1
  }
  count += (html.match(/<[^>]+(?:contenteditable=["']?true|role=["']?textbox)[^>]*(?:code|editor)[^>]*>/gi) || []).length
  count += (html.match(/<canvas\b[^>]*(?:editor|code)[^>]*>/gi) || []).length
  return count
}
...
function sameEditorType(a, b) {
  return detectEditorTypeFromNode(a) === detectEditorTypeFromNode(b)
}
...
function detectEditorTypeFromNode(node) {
...
function extractOptionsFromHtml(html) {
  const groups = [
    ...groupMatches(html, /<fieldset[^>]*>([\s\S]*?)<\/fieldset>/gi).map((html) => ({ html, relationship: 'strong' })),
    ...groupMatches(html, /<[^>]+role=["']?radiogroup["']?[^>]*>([\s\S]*?)<\/[^>]+>/gi).map((html) => ({ html, relationship: 'strong' })),
    ...groupMatches(html, /<[^>]+role=["']?listbox["']?[^>]*>([\s\S]*?)<\/[^>]+>/gi).map((html) => ({ html, relationship: 'explicit' })),
    ...groupMatches(html, /<(?:ul|ol)[^>]*>([\s\S]*?)<\/(?:ul|ol)>/gi).map((html) => ({ html, relationship: 'explicit' })),
    { html, relationship: 'explicit' },
  ]
  const best = groups
    .map((group) => ({ options: extractOptionGroupFromHtml(group.html), relationship: group.relationship }))
...
function extractOptionsFromDocument(root) {
...
function groupMatches(html, pattern) {
  return [...html.matchAll(pattern)].map((match) => match[1] || match[0])
}

function extractOptionGroupFromHtml(html) {
  const entries = []
  for (const match of html.matchAll(/<(label|li|div|span)[^>]*(?:role=["']?option["']?|[^>]*)>([\s\S]*?)<\/\1>/gi)) {
    const whole = match[0] || ''
    const inner = match[2] || ''
    if (isActionOrControlText(stripTags(inner)) || /<button\b|<select\b|<option\b/i.test(inner)) continue
    const text = stripTags(inner)
    if (isQuestionLikeOptionText(text)) continue
    if (isOptionText(text) || /<input\b[^>]*type=["']?(radio|checkbox)/i.test(inner) || /role=["']?option/i.test(whole)) {
      entries.push({
        ...normalizeOption(text, entries.length),
        input_present: /<input\b[^>]*type=["']?(radio|checkbox)/i.test(inner),
        role_option: /role=["']?option/i.test(whole),
      })
    }
  }
  return dedupeOptions(entries).filter((option) => option.text)
}
...
function isOptionText(text) {
  return /^[a-z0-9][).:-]\s+\S/i.test(sanitizeText(text, 320))
}
...
function extractExamplesFromHtml(html) {
  const examples = []
  const exampleBlocks = html.match(/<(section|div|article)[^>]*(?:example|sample)[^>]*>[\s\S]*?<\/\1>/gi) || []
  for (const block of exampleBlocks) {
    const label = matchFirst(block, /<(?:h[1-6]|strong|b|dt|p|div|span)[^>]*>\s*([^<]*(?:example|sample|test case)[^<]*)<\/(?:h[1-6]|strong|b|dt|p|div|span)>/i)
    examples.push(...parseExamples(stripTags(block), parseExampleHeading(label)))
  }
  return limitExamples(dedupeExamples(examples))
}

function extractExamplesFromDocument(root) {
  const nodes = Array.from(root?.querySelectorAll?.('[data-section*="example"],[class*="example"],[class*="sample"]') || []).filter(isVisible)
  return limitExamples(dedupeExamples(nodes.flatMap((node) => parseExamples(visibleText(node), parseExampleHeading(nodeLabel(node) || headingText(node))))))
}
...
function limitExamples(examples) {
  limitExamples.lastTruncated = examples.length > LIMITS.maxExamples
  if (limitExamples.lastTruncated) exampleDiagnosticsState.truncated = true
  return examples.slice(0, LIMITS.maxExamples)
}
...
function detectQuestionType({ statement, sections, code, options, visual }) {
  const text = `${statement}\n${sections.function_description || ''}`.toLowerCase()
  ...
  if (code || sections.input_format || sections.output_format || codingText) return 'coding'
...
function evaluateCompleteness({ questionType, statement, sections, code, options, signature, visual }) {
  const missing = []
  const warnings = []
  ...
  if (questionType === 'mcq') {
    ...
    if (sections.function_description && !code && !signature) warnings.push('code_context_partially_extracted')
  ...
  return {
    complete: missing.length === 0 && !(visual.image_context_required && !readable),
    missing,
    warnings: missing.length ? [...warnings, 'extraction_incomplete'] : warnings,
  }
}

function calculateConfidence({ questionType, statement, title, sections, code, options, visual, selection, complete, warnings = [] }) {
  let score = 0.35
  if (complete) score += 0.2
  if (statement) score += 0.15
  if (title) score += 0.08
  if (sections.input_format || sections.output_format || sections.constraints || sections.examples?.length) score += 0.08
  if (code || options.length >= 2) score += 0.1
  if (selection.selected?.score >= 8) score += 0.08
  if (questionType === 'unknown') score -= 0.18
  if (selection.ambiguous) score -= 0.18
...
function extractVisualContextFromHtml(html) {
  const safeHtml = String(html || '').replace(/<svg\b[^>]*(?:aria-hidden=["']?true["']?|role=["']?presentation["']?)[\s\S]*?<\/svg>/gi, ' ')
  const visualPresent = /<(img|figure|svg|canvas)\b/i.test(safeHtml)
  ...
  return {
    ...
    diagram_present: diagram,
    chart_present: chart,
    image_context_required: diagram || chart,
    visual_description: null,
  }
}

function extractVisualContextFromDocument(root) {
...
function hasFoldedEditorMarkup(html) {
  return /\b(?:folding-collapsed|folded|fold-placeholder|collapsed-fold|hidden-range|codicon-folding-collapsed)\b/i.test(String(html || ''))
}
...
function titleFromRegion(html) {
  const candidates = []
  for (const match of html.matchAll(/<(h[1-4])[^>]*>([\s\S]*?)<\/\1>/gi)) {
    const tag = (match[1] || '').toLowerCase()
    const text = stripTags(match[2] || '')
    if (labelKey(text)) continue
    if (text) candidates.push({ text, rank: tag === 'h1' ? 1 : tag === 'h2' ? 2 : 3, index: match.index || 0 })
  }
  for (const match of html.matchAll(/<([a-z0-9]+)[^>]*role=["']?heading["']?[^>]*>([\s\S]*?)<\/\1>/gi)) {
    const text = stripTags(match[2] || '')
    if (labelKey(text)) continue
    const rank = /aria-level=["']?1/i.test(match[0]) ? 1 : /aria-level=["']?2/i.test(match[0]) ? 2 : 3
    if (text) candidates.push({ text, rank, index: match.index || 0 })
  }
  for (const match of html.matchAll(/<(?:div|p|span|strong|b)[^>]*>([\s\S]*?)<\/(?:div|p|span|strong|b)>/gi)) {
    const text = stripTags(match[1] || '')
    if (labelKey(text)) continue
    if (isNumberedProblemTitle(text)) candidates.push({ text, rank: 2, index: match.index || 0 })
  }
  return rankTitleCandidates(candidates)?.text || null
}
...
function titleScore(candidate) {
  ...
  if (candidate.index < 800) score += 3
  if (candidate.index < 2400) score += 1
...
function cleanDocumentTitle(title) {
  const safe = sanitizeText(title, LIMITS.maxTitleLength)
  return safe.split(/\s[-|–—]\s/).map((part) => part.trim()).filter(Boolean)[0] || null
}

function pickStatement(text, title) {
  const safe = sanitizeText(text, LIMITS.maxTextLength)
  if (!title) return safe
  return sanitizeText(safe.replace(title, '').trim() || safe, LIMITS.maxTextLength)
}
...
function detectLanguageCandidate(code) {
  if (!code) return null
...
function detectEditorTypeFromText(code) {
  return code ? 'visible_code_region' : null
}

function splitLines(text) {
  return sanitizeText(text, LIMITS.maxSectionTextLength)
    .split(/(?:\n|;|\.\s+)/)
...
function extractConstraintsFromHtml(html, labelledText = '', code = '') {
  const chunks = []
  if (labelledText) chunks.push(labelledText)
  chunks.push(...constraintSectionsFromHtml(html))
  const labelled = finalizeConstraints(chunks, 'labelled')
  if (labelled.items.length) return labelled
  return finalizeConstraints([stripTags(stripConstraintFallbackHtml(stripEditorAndRuntimeHtml(html))).replace(code || '', '')], chunks.length ? 'labelled_plus_fallback' : 'fallback_clean_problem_text')
}

function extractConstraintsFromDocument(root, labelledText = '', code = '') {
  const chunks = []
  if (labelledText) chunks.push(labelledText)
  ...
  const labelled = finalizeConstraints(chunks, 'labelled')
  if (labelled.items.length) return labelled
  return finalizeConstraints([visibleTextWithoutEditorRuntime(root).replace(code || '', '')], chunks.length ? 'labelled_plus_fallback' : 'fallback_clean_problem_text')
}

function constraintSectionsFromHtml(html) {
  const cleaned = stripEditorAndRuntimeHtml(String(html || ''))
  const labelTag = '(?:h[1-6]|strong|b|dt|p|div|span)'
  const boundary = '(?:sample(?:\\s+input|\\s+output|\\s+explanation|\\s+\\d+)?|samples|example(?:\\s+input|\\s+output|\\s+explanation|\\s+\\d+)?|examples|input format|output format|function description|explanation|editorial|solution|discussion|run code|submit|custom input|test result)'
  const pattern = new RegExp(`<${labelTag}[^>]*>\\s*(?:constraints|limits)\\s*:?\\s*<\\/${labelTag}>([\\s\\S]*?)(?=<${labelTag}[^>]*>\\s*${boundary}\\s*:?\\s*<\\/${labelTag}>|$)`, 'gi')
  return [...cleaned.matchAll(pattern)].map((match) => stripTags(match[1] || '')).filter(Boolean)
}

function stripEditorAndRuntimeHtml(html) {
  let output = String(html || '')
  const block = new RegExp(`<([a-z][\\w:-]*)[^>]+(?:class|id|role|aria-label|data-testid|data-cy)=["'][^"']*${EDITOR_RUNTIME_MARKER}[^"']*["'][^>]*>[\\s\\S]*?<\\/\\1>`, 'gi')
  for (let i = 0; i < 4; i += 1) {
    output = output.replace(block, () => {
      exampleDiagnosticsState.editorSectionExcludedCount += 1
      return ' '
    })
  }
  return output
...
function stripConstraintFallbackHtml(html) {
  const labelTag = '(?:h[1-6]|strong|b|dt|p|div|span)'
  const noisy = '(?:sample(?:\\s+input|\\s+output|\\s+explanation|\\s+\\d+)?|samples|example(?:\\s+input|\\s+output|\\s+explanation|\\s+\\d+)?|examples|custom input|test result|run output|console|editorial|solution|discussion)'
  const boundary = '(?:problem|statement|question|description|instructions?|task|function description|input format|output format|constraints|limits)'
  const pattern = new RegExp(`<${labelTag}[^>]*>\\s*${noisy}\\s*:?\\s*<\\/${labelTag}>[\\s\\S]*?(?=<${labelTag}[^>]*>\\s*${boundary}\\s*:?\\s*<\\/${labelTag}>|$)`, 'gi')
...
function visibleTextWithoutEditorRuntime(root) {
  ...
  clone.querySelectorAll?.(`${EDITOR_RUNTIME_SELECTOR},pre,code,[class*="code"]`).forEach((entry) => entry.remove())
...
function stripTags(html) {
  const protectedHtml = String(html ?? '')
    .replace(/<=/g, '__SAIIA_LTE__')
    .replace(/>=/g, '__SAIIA_GTE__')
  return sanitizeTextPreserveTabs(
    protectedHtml
      .replace(/<br\s*\/?>/gi, '\n')
      .replace(/<\/(td|th)>/gi, '\t')
      .replace(/<\/(p|div|section|li|h[1-6]|pre|dt|dd|tr)>/gi, '\n')
      .replace(/<[^>]+>/g, ' ')
      .replace(/__SAIIA_LTE__/g, '<=')
      .replace(/__SAIIA_GTE__/g, '>=')
      .replace(/&nbsp;/g, ' ')
      .replace(/&lt;/g, '<')
      .replace(/&gt;/g, '>')
      .replace(/&amp;/g, '&')
...
function stripCodeTags(html) {
  return sanitizeCode(
    decodeCodeFragment(html)
  )
}

function decodeCodeFragment(html) {
  return String(html ?? '')
    .replace(/<br\s*\/?>/gi, '\n')
    .replace(/<\/(div|p|pre|code|span)>/gi, '\n')
    .replace(/<[^>]+>/g, '')
    .replace(/&nbsp;/g, ' ')
    .replace(/&lt;/g, '<')
    .replace(/&gt;/g, '>')
...
function visibleText(node) {
  ...
  if (clone?.querySelectorAll) {
    clone.querySelectorAll('script,style,noscript,template,nav,aside,footer,header,[hidden],[aria-hidden="true"],input[type="password"]').forEach((entry) => entry.remove())
  }
  return sanitizeText(clone?.textContent || node.textContent || '', LIMITS.maxTextLength)
}
...
function isVisible(node) {
  ...
  if (rect && rect.width === 0 && rect.height === 0 && !node.textContent?.trim()) return false
  return true
}

function viewportBonus(node) {
  const rect = node?.getBoundingClientRect?.()
  if (!rect) return 0
  if (rect.width <= 0 || rect.height <= 0) return -1
  const centerBias = Math.abs((rect.left + rect.width / 2) - (globalThis.innerWidth || 1200) / 2)
  return centerBias < (globalThis.innerWidth || 1200) * 0.35 ? 1 : 0
}

function normalizeQuestionType(type) {
  return QUESTION_TYPES.includes(type) ? type : 'unknown'
}

function boundedUnique(values) {
  return [...new Set(values.filter(Boolean))].slice(0, LIMITS.maxWarnings)
}
