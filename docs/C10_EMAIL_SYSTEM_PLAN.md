...
- **C10.5B - Feature email trigger wiring:** not started; authenticated user actions and backend trigger wiring remain deferred.
- **C10.5 - Session summary, transcript, and AI notes emails:** incomplete; C10.5A template/helper groundwork is complete locally, while C10.5B trigger wiring, authenticated actions, and production delivery remain deferred.
...
C10.1, C10.3A, C10.3B, C10.3C, C10.3D, C10.4A, C10.4B, and C10.5A do not call Resend, configure Supabase SMTP, add real API keys, send emails, create custom verification/reset tokens, modify applied migrations, or implement promotional messaging. C9 remains merged/closed. C10.2A runbook/setup documentation is completed/merged, but C10.2B live verification/reset delivery is blocked pending a verified Resend sender/domain, so C10.2 delivery is not complete. C10.3A is completed locally with disabled/offline dry-run defaults. C10.3B adds the backend-owned `outbound_email_events` migration and event-store boundary, and C10.3D records its successful remote dev apply and post-apply validation. C10.3C connects the event store to the dry-run provider only. C10.4A adds the service-level welcome template and C10.4B wires it after authenticated profile bootstrap; C10.5A adds feature templates/helpers only. All remain dry-run-only and non-blocking, with no automatic C10.5 triggers. Full C10.3/C10.5 and real C10.4/C10.5 delivery remain incomplete because real delivery is not enabled.
